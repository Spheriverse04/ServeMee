// frontend/src/app/my-provider-bookings/page.tsx
'use client';
...
interface Booking {
  ...
  consumer: { // Details of the user who made the booking
    id: string;
    email: string;
    displayName?: string;
  };
  service: { // Details of the service booked
    id: string;
    name: string;
    description: string;
    price: number;
    category?: string;
    // provider details are implicitly current user, so not strictly needed here
  };
}
...
export default function MyProviderBookingsPage() {
  ...
  const fetchProviderBookings = async () => {
    ...
    try {
      ...
      // This endpoint fetches bookings relevant to the authenticated user's role
      // For a SERVICE_PROVIDER, your backend will return bookings for their services.
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || 'Failed to fetch provider bookings.');
      }

      const data = await response.json();
      setBookings(data.bookings); // Assuming the backend returns an object with a 'bookings' array
    } catch (err: any) {
  ...
  // Generic handler for booking status updates
  const handleBookingAction = async (bookingId: string, action: 'confirm' | 'reject' | 'complete') => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:3000/bookings/${bookingId}/${action}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
      ...
      const data = await response.json();
      setActionMessage(`Booking ${action}ed successfully!`);
      ...
      setActionMessage(err.message || `An unexpected error occurred while ${action}ing the booking.`);
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-100">
        <p className="text-lg text-gray-700">Loading bookings for your services...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-4xl mx-auto">
        ...
        {bookings.length === 0 ? (
          <p className="text-center text-gray-600 text-xl mt-10">No bookings for your services yet.</p>
        ) : (
              ...
              >
                <div className="flex-1 mb-4 sm:mb-0">
                  ...
                  <p className="text-gray-600 text-sm mb-1">
                    ...
                    {booking.consumer?.displayName || booking.consumer?.email}
                  </p>
                  <p className="text-gray-600 text-sm mb-1">
                ...
                <div className="flex-shrink-0 flex flex-col items-end sm:items-center space-y-2">
                  <span
                    className={`inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium ${getStatusBadgeColor(booking.status)}`}
                  >
                  ...
                  {booking.status === 'CONFIRMED' && (
                    <button
                      ...
                      className="px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 mt-2"
                    >
                      Mark as Completed
                    </button>
                  )}
